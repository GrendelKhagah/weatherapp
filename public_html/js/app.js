// Main frontend controller for WeatherApp.
(() => {
const {
  el,
  fmtNum,
...
const { createCharts } = window.App.charts;
const { createHistoryHandlers } = window.App.history;
const { createMlHandlers, sortMlForecastRows } = window.App.ml;
const { createUiHandlers } = window.App.ui;
  ...
  const mlWeatherModel = el("mlWeatherModel");
  const mlForecastStatus = el("mlForecastStatus");
  const mlForecastLegend = el("mlForecastLegend");
  const mlForecastChart = el("mlForecastChart");
  const mlForecastModel = el("mlForecastModel");
  ...
  let selectedMlForecastModel = "all";

  const charts = createCharts(
    { hourlyChart, historyChart, mlForecastChart },
    { fmtNum, formatShortDate }
  );
  const { renderHourlyChart, renderHistoryChart, renderMlForecastChart } = charts;

  apiBaseLabel.textContent = window.API.base || "(missing CONFIG.API_BASE)";

  const uiHandlers = createUiHandlers({
    utilStations,
    utilStationsWithData,
    utilGridpoints,
    utilTrackedPoints,
    utilApiHealth,
  });
  const { refreshUtilityCounts, refreshExternalApiHealth, initPanelCollapsing } = uiHandlers;
  ...
  async function loadTemperatureLayer() {
    const hours = Number(tempHourOffset?.value || 0);
    const bbox = window.API.bboxFromMap(map);
    const gj = await window.API.layerTemperature(hours, bbox);
  ...
  map.on("load", async () => {
    ...
    // Temperature: interpolated surface from stations
    map.addLayer({
      id: "temp-heat",
      type: "circle",
      source: SRC.temp,
      ...
      paint: {
        ...
        "circle-color": [
          "interpolate",
          ["linear"],
          ["coalesce", ["get", "temperature_c"], 15],
          -5, "rgba(80,160,255,0.85)",
          10, "rgba(100,210,220,0.85)",
          20, "rgba(255,200,90,0.85)",
          30, "rgba(255,120,90,0.9)"
        ]
      }
    });
  ...
  if (mlForecastModel) {
    mlForecastModel.addEventListener("change", () => {
      selectedMlForecastModel = mlForecastModel.value || "all";
      const filtered = filterMlForecastByModel(lastMlForecastRows, selectedMlForecastModel);
      renderMlForecastChart(filtered, lastMlForecastNwsRows || []);
      renderMlForecastDetails(filtered);
    });
  }
  ...
  let mlForecastResizeTimer = null;
  ...
  if (typeof ResizeObserver !== "undefined") {
    if (historyChart) {
      const historyObserver = new ResizeObserver(() => {
        clearTimeout(historyResizeTimer);
        historyResizeTimer = setTimeout(() => {
          if (lastHistoryRows.length) renderHistoryChart(lastHistoryRows);
        }, 150);
      });
      historyObserver.observe(historyChart);
    }
    if (mlForecastChart) {
      const mlObserver = new ResizeObserver(() => {
        clearTimeout(mlForecastResizeTimer);
        mlForecastResizeTimer = setTimeout(() => {
          if (lastMlForecastRows.length) {
            const filtered = filterMlForecastByModel(lastMlForecastRows, selectedMlForecastModel);
            renderMlForecastChart(filtered, lastMlForecastNwsRows || []);
          }
      ...
      mlObserver.observe(mlForecastChart);
    }
  }
  ...
  function clearTables() {
    ...
    if (mlWeatherModel) mlWeatherModel.textContent = "--";
    if (mlForecastStatus) mlForecastStatus.textContent = "No ML forecast yet.";
    if (mlForecastLegend) mlForecastLegend.innerHTML = "";
  ...
  async function refreshHourlyForSelected() {
    ...
    try {
      const hourlyLive = await window.API.pointHourlyList(lat, lon, 24, true);
  ...
  const { fetchHistoryForStation, loadHistoryForStation, loadHistoryFromNearestStations } = historyHandlers;

  function setMlForecastData(rows, nwsRows) {
    lastMlForecastRows = sortMlForecastRows(rows);
    lastMlForecastNwsRows = Array.isArray(nwsRows) ? nwsRows : [];
    updateMlForecastModelOptions(lastMlForecastRows);
    const filtered = filterMlForecastByModel(lastMlForecastRows, selectedMlForecastModel);
    renderMlForecastChart(filtered, lastMlForecastNwsRows || []);
    renderMlForecastDetails(filtered);
  }

  function mlForecastModelKey(row) {
    if (!row) return "";
  ...
  function mlForecastModelLabel(row) {
    if (!row) return "model";
    const name = row.model_name || "model";
    const detail = row.model_detail || "";
    return detail ? `${name} (${detail})` : name;
  ...
  function updateMlForecastModelOptions(rows) {
    if (!mlForecastModel) return;
    const list = Array.isArray(rows) ? rows : [];
    const options = new Map();
    list.forEach(r => {
      const key = mlForecastModelKey(r);
      if (!key) return;
      if (!options.has(key)) options.set(key, mlForecastModelLabel(r));
    });

    const prev = selectedMlForecastModel;
    mlForecastModel.innerHTML = "";
    const optAll = document.createElement("option");
    optAll.value = "all";
    optAll.textContent = "All models";
    mlForecastModel.appendChild(optAll);
    options.forEach((label, key) => {
      const opt = document.createElement("option");
      opt.value = key;
      opt.textContent = label;
      mlForecastModel.appendChild(opt);
    });

    if (prev && (prev === "all" || options.has(prev))) {
      mlForecastModel.value = prev;
    } else {
      mlForecastModel.value = "all";
      selectedMlForecastModel = "all";
    }
  }
  ...
  function averageRows(rows, modelLabel) {
    const list = Array.isArray(rows) ? rows : [];
    const sum = {
      tmin_c: 0,
      tmax_c: 0,
    ...
    list.forEach(r => {
      if (Number.isFinite(r.tmin_c)) { sum.tmin_c += r.tmin_c; sum.tmin_n += 1; }
      if (Number.isFinite(r.tmax_c)) { sum.tmax_c += r.tmax_c; sum.tmax_n += 1; }
      if (Number.isFinite(r.tmean_c)) { sum.tmean_c += r.tmean_c; sum.tmean_n += 1; }
      if (Number.isFinite(r.prcp_mm)) { sum.prcp_mm += r.prcp_mm; sum.prcp_n += 1; }
      if (Number.isFinite(r.confidence)) { sum.confidence += r.confidence; sum.conf_n += 1; }
    });
    return {
      as_of_date: list[0]?.as_of_date || null,
      horizon_hours: list[0]?.horizon_hours || 0,
      tmin_c: sum.tmin_n ? sum.tmin_c / sum.tmin_n : null,
      tmax_c: sum.tmax_n ? sum.tmax_c / sum.tmax_n : null,
      tmean_c: sum.tmean_n ? sum.tmean_c / sum.tmean_n : null,
      prcp_mm: sum.prcp_n ? sum.prcp_mm / sum.prcp_n : null,
      confidence: sum.conf_n ? sum.confidence / sum.conf_n : null,
      model_name: modelLabel,
      model_detail: null,
    };
  }

  function aggregateByModelAndDay(rows) {
    const list = Array.isArray(rows) ? rows : [];
    const grouped = new Map();
    list.forEach(r => {
      const date = dayKey(r);
      const model = mlForecastModelKey(r);
    ...
    grouped.forEach((items, key) => {
      const modelLabel = mlForecastModelLabel(items[0]);
      const avg = averageRows(items, modelLabel);
      out.push({ ...avg, _day: key.split("::")[0] });
    });
    out.sort((a, b) => (a._day || "").localeCompare(b._day || ""));
    return out;
  }

  function aggregateAllModels(rows) {
    const perModelDay = aggregateByModelAndDay(rows);
    const byDay = new Map();
    perModelDay.forEach(r => {
      const date = r._day || dayKey(r);
      if (!date) return;
    ...
    byDay.forEach((items, date) => {
      const avg = averageRows(items, "All models (avg)");
      avg._day = date;
      out.push(avg);
    });
    out.sort((a, b) => (a._day || "").localeCompare(b._day || ""));
  ...
  function filterMlForecastByModel(rows, modelKey) {
    const list = Array.isArray(rows) ? rows : [];
    if (!modelKey || modelKey === "all") return aggregateAllModels(list);
    const filtered = list.filter(r => mlForecastModelKey(r) === modelKey);
    return aggregateByModelAndDay(filtered);
  }

  function renderMlForecastDetails(rows) {
    if (!mlStatus || !mlTbody) return;
    const list = Array.isArray(rows) ? rows.slice(0, 10) : [];
    if (!list.length) {
      mlStatus.textContent = "No ML forecast details yet.";
      mlTbody.innerHTML = "";
      return;
    }

    mlStatus.textContent = `rows=${list.length}`;
    mlTbody.innerHTML = list.map(r => {
      const base = r.as_of_date ? new Date(`${r.as_of_date}T00:00:00Z`) : null;
      const day = base ? new Date(base.getTime() + (r.horizon_hours || 0) * 3600000) : null;
      const label = day ? day.toISOString().slice(0, 10) : "";
      const modelLabel = r.model_name ? r.model_name : mlForecastModelLabel(r);
      const conf = Number.isFinite(r.confidence) ? `${Math.round(r.confidence * 100)}%` : "--";
      return `<tr>
        <td>${label}</td>
  ...
  const mlHandlers = createMlHandlers({
    elements: {
      mlWeatherStatus,
      mlWeatherTodayDate,
    ...
    helpers: { fmtNum, formatShortDate },
    getHistoryRows: () => lastHistoryRows,
    getNearestStations: () => lastNearestStations,
    setMlForecastData,
  });
  const { setMlWeatherData, updateMlWeather } = mlHandlers;
  ...
  function renderHourlyForecast(rows, emptyMessage) {
    ...
    const now = new Date();
    let startIndex = list.findIndex(r => {
      const start = r.start_time ? new Date(r.start_time) : null;
      const end = r.end_time ? new Date(r.end_time) : null;
      if (!start || Number.isNaN(start.getTime())) return false;
      if (end && !Number.isNaN(end.getTime())) return now < end;
      return now <= start;
    });
    if (startIndex < 0) {
      hourlySummary.textContent = emptyMessage || "No hourly forecast for the current time";
      hourlyChart.innerHTML = "";
      hourlyTbody.innerHTML = "";
      setHourlyData([]);
      return;
    ...
    const next24 = list.slice(startIndex, startIndex + 24);
  ...
  async function loadDetailsForPoint(lat, lon, label, stationOverride, contextType = "pin", sourceId) {
    ...
    try {
      ...
      const data = await window.API.pointSummary(lat, lon, days, 25);
        ...
        let gridRef = null;
        try {
          const hourlyLive = await window.API.pointHourlyList(lat, lon, 24, false);
          const list = hourlyLive && hourlyLive.periods ? hourlyLive.periods : [];
          renderHourlyForecast(list);
  ...
  async function loadDetailsForGrid(gridId, metaText, lat, lon) {
    ...
    // ML forecast details handled by setMlForecastData
  }
